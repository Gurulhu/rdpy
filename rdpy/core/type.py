...
class Type(object):
    ...
    def read(self, s):
        ...
        if old.__getValue__() != self.__getValue__():
            #rollback read value
            s.pos -= sizeof(self)
...
class SimpleType(Type, CallableValue):
    ...
    def __setValue__(self, value):
        ...
        """
        #Coerces byte values to int
        if type(value) == type(bytes()):
            value = struct.unpack(self._structFormat, value)
            print("read", self._structFormat, value)
            value = value[0]
    ...
    def __write__(self, s):
        ...
        @param s: Stream that will be written
        """
        print("write", type(self.value), self.value, self._structFormat)
        if type(self.value) == type(float()):
            s.write(bytearray(struct.pack("f", self.value)))
        else:
            s.write(bytearray(struct.pack(self._structFormat, self.value)))
    ...
    def __read__(self, s):
        ...
        self.value = s.read(self._typeSize)
    ...
    def isInRange(self, value):
        ...
        if self._signed:
            ...
            return not ( ( value < 0 ) | ( value > self.mask() ) )
